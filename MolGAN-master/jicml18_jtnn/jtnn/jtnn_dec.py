...
class JTNNDecoder(nn.Module):
    ...
    def forward(self, mol_batch, mol_vec):
        ...
        # Predict Root
        pred_hiddens.append(create_var(
        ...
        for t in range(max_iter):
            ...
            # Message passing
            cur_h_nei = torch.stack(
                cur_h_nei, dim=0).view(-1, MAX_NB, self.hidden_size)
            new_h = GRU(cur_x, cur_h_nei, self.W_z,
                        self.W_r, self.U_r, self.W_h)

            # Node Aggregate
            cur_o_nei = torch.stack(
                cur_o_nei, dim=0).view(-1, MAX_NB, self.hidden_size)
        ...
        for step in range(MAX_DECODE_LEN):
            ...
            if not backtrack:  # Forward: Predict next clique
                ...
                if prob_decode:
                    ...
                    _, sort_wid = torch.sort(
                        pred_score, dim=1, descending=True)
